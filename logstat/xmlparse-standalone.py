...
xmlslist=[join(xmlpath, f) for f in onlyxml]
...
for xml in xmlslist:
    ...
    mellanoxlist = getdata(xml, classname='IBMSG_PCIRawData', name='RawResult',
                           rawsearch='Mellanox Technologies Device')
    ...
    reportfile = open(os.path.join(xmlpath, xml)+'.txt', "w")
